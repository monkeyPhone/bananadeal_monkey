...
class StopWatchTimer {
  ...
  int _getCountUpTime(int presetTime) =>
      DateTimeConfig().now.millisecondsSinceEpoch -
          _startTime +
          _stopTime +
          presetTime;
  ...
  int _getCountDownTime(int presetTime) => max(
    presetTime -
        (DateTimeConfig().now.millisecondsSinceEpoch - _startTime + _stopTime),
    0,
  );

  void _start() {
    if (!isRunning) {
      _startTime = DateTimeConfig().now.millisecondsSinceEpoch;
  ...
  bool _stop() {
    if (isRunning) {
      _timer?.cancel();
      _timer = null;
      _stopTime += DateTimeConfig().now.millisecondsSinceEpoch - _startTime;
