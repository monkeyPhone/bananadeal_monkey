...
part 'banana_reset_event.dart';
part 'banana_reset_state.dart';

class BananaResetBloc extends Bloc<BananaResetEvent, BananaResetState> {
  final ResetRepository _repository;
  late Timer? _timer;
  BananaResetBloc({
    required ResetRepository repository,
  }) : _repository = repository, super(const BananaResetState()) {
    ...
    on<FinishReset>(_finishReset);
  }

  Future<void> _loading(Loading event, Emitter<BananaResetState> emit) async{
  ...
  Future<void> _sendEmail(SendEmail event, Emitter<BananaResetState> emit) async {
    add(const Loading());
    await emit.forEach(
        _repository.streamFindEmail(email: state.email, ),
  ...
  Future<void> _initialSendEmail(InitialSendEmail event, Emitter<BananaResetState> emit) async {
    add(const InitCount());
    emit(state.copyWith(dataDto: ModelConfig.dataDto, originOtp: '', otpProgressEnum: OtpProgressEnum.email, otp: ''));
  }

  Future<void> _resendEmail(ResendEmail event, Emitter<BananaResetState> emit) async {
    add(const InitCount());
    emit(state.copyWith(dataDto: ModelConfig.dataDto, originOtp: '', otp: ''));
    add(const SendEmail());
  }

  Future<void> _fetchOtp(FetchOtp event,  Emitter<BananaResetState> emit) async{
    emit(state.copyWith(otp: event.otp));
  }

  Future<void> _fetchEmailValidate(FetchEmailValidate event, Emitter<BananaResetState> emit) async {
    emit(state.copyWith(validate: event.validate, email: event.email));
  }

  Future<void> _initCount(InitCount event, Emitter<BananaResetState> emit) async{
    add(const CancelCount());
    emit(state.copyWith(count: 300));
  }

  Future<void> _minusCount(MinusCount event, Emitter<BananaResetState> emit) async {
    int count = state.count;
    count--;
    emit(state.copyWith(count: count));
  }

  Future<void> _startCount(StartCount event, Emitter<BananaResetState> emit) async{
    emit(state.copyWith(count: 300));
  ...
  Future<void> _cancelCount(CancelCount event, Emitter<BananaResetState> emit) async{
    try{
      if(_timer != null){
        if(_timer!.isActive){
          _timer!.cancel();
  ...
  Future<void> _changePw(ChangePw event, Emitter<BananaResetState> emit) async{
    add(const Loading());
    add(const CancelCount());
    emit(state.copyWith(
        dataDto: ModelConfig.dataDto,
  ...
  Future<void> _fetchPw1(FetchPw1 event, Emitter<BananaResetState> emit) async{
    emit(state.copyWith(pw1: event.pw1, validate: event.validate));
  }

  Future<void> _fetchPw2(FetchPw2 event, Emitter<BananaResetState> emit) async{
    emit(state.copyWith(pw2: event.pw2, validate: event.validate));
  }

  Future<void> _fetchLock1(FetchLock1 event, Emitter<BananaResetState> emit) async{
    bool lock = !state.lock1;
    emit(state.copyWith(lock1: lock));
  }

  Future<void> _fetchLock2(FetchLock2 event, Emitter<BananaResetState> emit) async{
    bool lock = !state.lock2;
    emit(state.copyWith(lock2: lock));
  }

  Future<void> _finishReset(FinishReset event, Emitter<BananaResetState> emit) async{
    add(const Loading());
    await emit.forEach(_repository.streamResetPw(
        email: state.email,
        pw: state.pw1
    ), onData: (result) => state.copyWith(dataDto: result));
    if(state.dataDto.statusEnum == StatusEnum.success){
